...
//	text.append(s_load_inflows());
	...
	*/
	...
	for each (QString line in text)
		r.append(line.replace("Yes", "1").replace("No", "0"));
	return r;
}
...
{
	QStringList r;
	r.append("//Global/Solver Settings");
	QList<Entity*> list;
	list.append(entitiesByType("Global Settings"));
	list.append(entitiesByType("Solver"));
...
//		b.append(QString("; a=%1; depth=%2").arg(n->val("a")).arg(n->val("depth")));
		...
		for each (QString code in list)
		{
			if (code == "inflow" && !n->val(code).isEmpty() && n->val(code) != ".")
				b.append(QString("; inflow=%1").arg(fullFilename(n->val(code), modelPathname())));
			else if (!n->val(code).isEmpty() && n->val(code) != "Default") b.append(QString("; %1=%2").arg(code).arg(n->val(code)));
	...
	{
		QString c = "Connector=";
		c.append(e->objectType.ObjectType);
		c.append(", {id=");
		c.append(e->Name());
		...
		list.removeOne("id");
		for each (QString code in list)
			if (!e->val(code).isEmpty() && e->val(code) != "Default") c.append(QString("; %1=%2").arg(code).arg(e->val(code)));
		c.append("}");
		log(c);
		r.append(c);
	}
...
QStringList GraphWidget::s_get_external_flux() const
{
	...
	for each (Entity *e in entitiesByType("External FLux"))
	{
		...
		f.append(e->Name());
		f.append(", {model=");
		f.append(e->props.list["Model"]);
		QStringList list = e->codes();
		list.removeOne("Name");
		list.removeOne("Model");
		for each (QString code in list)
			//		for each (QString code in e->props.list.keys())
			if (!e->val(code).isEmpty() && e->val(code) != "Default") f.append(QString("; %1=%2").arg(code).arg(e->val(code)));
		f.append("}");
		log(f);
		r.append(f);
	}
