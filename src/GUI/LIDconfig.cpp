...
void CLIDconfig::getconfigfromfile(string filename)
{
		...
		while (file.eof() == false)
		{
				...
				if (s.size() > 0)
				{
					...
					if (ss.size() > 1)
					{
...

						{
							...
							for (unsigned int i = 1; i < ss2.size(); i++) params.push_back(ss2[i]);
							open_bracket = true;

						}
						if ((int(s.size()) > 1) && (look_up(s[1], '}').size() > 0))
			...
			else if (open_bracket == true)
			{
				...
				if (s.size()>0)
				{
					...
					for (unsigned int i = ii; i<ss2.size(); i++) params.push_back(ss2[i]);
	...
	for (unsigned int i = 0; i<keyword.size(); i++)
	{
		...
		for (unsigned int j = 0; j<param_list[i].size(); j++)
		{
			if (param_list[i][j] != "")
			{
...
CLIDconfig CLIDconfig::extract_subset(int n)
{
	...
	for (unsigned int i = 0; i< keyword.size(); i++)
	{
		if ((experiment_id_keyword[i] == "") || (lookupexperiments(experiment_id_keyword[i]) == n))
		{
			...
			for (unsigned int j = 0; j < param_names[i].size(); j++)
			{
...
{
	for (unsigned int i = 0; i < experiment_names.size(); i++)
